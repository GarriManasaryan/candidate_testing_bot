...
# google service
path_to_google_drive_service_account_json = os.path.join(
    os.getcwd(), "configs", "google_service_credentials.json"
)
...
with open(
    os.path.join(os.getcwd(), "spam_defender_files", "already_processed_users.json")
) as f:
...
banned_message = (
    f"Sorry, but you've been banned for spamming; {further_instructions_message}"
)
...
error_downloading_files = (
    f"Something went wrong while donwloading your tasks; {further_instructions_message}"
)
...
instruction_all_answers = (
    "All answers should be submitted in the same file and sent back to this bot"
)
...
full_task_instr = "\n\n".join(
    [
        instruction_start,
        instruction_clinical_part,
        instruction_excel,
        instruction_sending_answers,
    ]
)
just_clinical_task_instr = "\n\n".join(
    [instruction_start, instruction_clinical_part, instruction_sending_answers]
)
just_excel_instr = "\n\n".join(
    [instruction_start, instruction_excel, instruction_sending_answers]
)
...
# reminders
time_left_base_message = (
    "Warning! You have less than <b>TIME_LEFT minutes</b> to submit an excel task"
)
...
# customs
start_already_pushed = (
    "Your test has already started, use the buttons above to submit answers"
)
...
processing_files_upload = "Well done! Processing files (~1 min)..."
final_success = "All files are processed successfully! Your HR coordinator will get in touch with you very soon; have a lovely day :)"
