...
instruction_time_limit = 'Time limit:'
instruction_clinical_part = f'• <u>Clinical part</u>: theoretical questions in a cancer-specific field.\n{instruction_time_limit} <b>1 week</b> (approximately)'
instruction_excel = f'• <u>Excel basics</u>: you will be given an excel file with a series of practice tasks. For example, calculate duplicated values in the table or filter data using formulas etc.\n{instruction_time_limit} <b>4 hours</b>'
instruction_sending_answers = '<i>How to send back files with answers?</i>\n\nA message will be sent to you with a button "Submit answers": click it, choose task and send the file. If you have several files, you need to click "Submit answers" for every file separately and send them one at a time. Make sure, you get the response from bot (successfully submitted)'
